...
3. CRITICAL: Never mention a player by name unless that name appears verbatim in the data below. You do not know who is on this team beyond what is in the data.
...
function buildUserMessage(
  pageType: string,
  focusAreas: string[],
  context: Awaited<ReturnType<typeof buildConciergeContext>>,
...
export async function POST(request: Request) {
  try {
    ...
    const userMessage = buildUserMessage(pageType, resolvedFocus, context, customQuestion, resolvedStyle)
    ...
    const ollamaRes = await fetch(`${OLLAMA_BASE}/api/generate`, {
      ...
      body: JSON.stringify({
        ...
        stream: false,
        options: {
          temperature: getTemperature(resolvedStyle),
          num_predict: 8192,
        },
      }),
    })
    ...
    if (!ollamaRes.ok) {
      ...
      console.error('[concierge] Ollama HTTP error:', ollamaRes.status, errText.slice(0, 500))
      return NextResponse.json({ error: `Ollama API error: ${ollamaRes.status}` }, { status: 502 })
    }
    ...
    return NextResponse.json({
      content: cleaned || 'No analysis could be generated with the available data.',
